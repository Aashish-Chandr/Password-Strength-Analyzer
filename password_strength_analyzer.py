...
# ---------------------------------------------------------------------------
# Generator — cryptographically secure password suggestion engine
# ---------------------------------------------------------------------------

class Generator:
    """Produces cryptographically secure password suggestions.

    Every password generated by this class:
    - Contains at least one character from each of the four required
      complexity categories (uppercase, lowercase, digit, special character).
    - Has a total length of 20 characters by default (configurable via the
      ``length`` parameter of ``_build_password``).
    - Is built exclusively using Python's ``secrets`` module — the
      cryptographically secure random source designed specifically for
      generating credentials, tokens, and other security-sensitive values.

    ``secrets`` vs ``random``
    -------------------------
    Python's ``random`` module uses a Mersenne Twister PRNG whose state can
    theoretically be reconstructed from observed output.  ``secrets`` uses
    the operating system's cryptographically secure RNG (e.g., ``/dev/urandom``
    on Unix, ``CryptGenRandom`` on Windows), which is specifically designed
    to be unpredictable and resistant to state-reconstruction attacks.  For
    any credential generation, ``secrets`` is the correct choice.
    """

    def __init__(self, weak_list: list[str] | None = None) -> None:
        """Initialise the Generator with an optional weak-password list.

        Parameters
        ----------
        weak_list : list[str] | None
            A list of commonly-used weak passwords (lowercase).  When ``None``
            the module-level ``WEAK_PASSWORD_LIST`` is used.  This parameter
            exists to allow callers to inject a custom list without modifying
            global state, which is important for isolated testing.
        """
        # Use the caller-supplied list, or fall back to the module constant.
        self._weak_list: list[str] = weak_list if weak_list is not None else WEAK_PASSWORD_LIST
    ...
    def generate_suggestions(self, n: int = 3) -> list[str]:
        """Return exactly *n* cryptographically secure password suggestions.

        Each suggestion is produced by ``_build_password()`` and subjected to
        a one-retry weak-list check via ``_passes_weak_check()``.  If the
        first candidate matches the weak list, one replacement attempt is made;
        if the replacement also matches, it is returned as-is (the probability
        of two consecutive 20-character random passwords both appearing in the
        weak list is astronomically small).

        Parameters
        ----------
        n : int
            The number of password suggestions to produce (default: 3).

        Returns
        -------
        list[str]
            Exactly ``n`` password strings, each satisfying the four complexity
            categories and the length constraints defined in ``_build_password``.
        """
        suggestions: list[str] = []

        for _ in range(n):
            # Generate the first candidate using the cryptographically secure
            # _build_password() method.
            candidate = self._build_password()

            if not self._passes_weak_check(candidate):
                # First candidate matched the weak list — generate one
                # replacement attempt.  Per Requirement 2.5, if this
                # replacement also matches the weak list, we return it as-is
                # without a further check.  This bounds the retries at 2 total
                # attempts per slot, preventing any possibility of an infinite
                # loop (however unlikely the trigger condition is in practice).
                candidate = self._build_password()
                # No further check: accept this replacement regardless of
                # whether it matches the weak list (Requirement 2.5).
    ...
    def _build_password(self, length: int = 20) -> str:
        """Construct a single cryptographically secure password of *length* chars.

        The algorithm proceeds in three stages:
        1. **Seed stage**: pick one character from each of the four required
           complexity categories (uppercase, lowercase, digit, special) using
           ``secrets.choice``.  This guarantees all four categories are always
           present, regardless of the random fill in step 2.
        2. **Fill stage**: fill the remaining ``length - 4`` slots by selecting
           characters from the full printable-ASCII alphabet
           (``string.ascii_letters + string.digits + string.punctuation``)
           using ``secrets.choice``.  Using the full alphabet for the fill
           characters maximises the password's entropy.
        3. **Shuffle stage**: shuffle the combined list using
           ``secrets.SystemRandom().shuffle``, which uses the OS CSPRNG as the
           entropy source.  This distributes the seeded category characters
           randomly throughout the password rather than clustering them at the
           start, preventing any predictable positional pattern.

        Why ``secrets``?
        ----------------
        ``secrets.choice`` draws from the OS cryptographically secure RNG
        (``/dev/urandom`` on Unix, ``CryptGenRandom`` on Windows).  Unlike
        ``random.choice``, the output is not predictable from prior outputs,
        making the generated passwords safe for use as credentials.

        Parameters
        ----------
        length : int
            Total number of characters in the generated password.  Must be ≥ 4
            (to accommodate one character per required category).  The design
            specifies a default of 20.

        Returns
        -------
        str
            A password string of exactly ``length`` characters containing at
            least one uppercase letter, one lowercase letter, one digit, and
            one special character (printable ASCII non-alphanumeric).
        """
        if length < 4:
            # Guard against misconfiguration: we need at least 4 slots for the
            # four required category characters.
            raise ValueError(
                f"_build_password: length must be at least 4, got {length}"
            )

        # ------------------------------------------------------------------
        # Stage 1 — Seed: guarantee at least one character from each required
        # complexity category using secrets.choice on category-specific subsets.
        #
        # Using secrets.choice here (rather than random.choice) ensures the
        # seeded characters are drawn from a cryptographically unpredictable
        # source — no adversary can predict which character from each category
        # was chosen based on timing or prior output.
        # ------------------------------------------------------------------

        # Category subsets.  string.punctuation covers printable ASCII
        # characters that are neither letters nor digits — exactly the
        # "special character" definition used throughout this project.
        ...
        # Pick one character from each category.  secrets.choice is
        # equivalent to random.choice but backed by the OS CSPRNG, making
        # each selection cryptographically unpredictable.
        chars: list[str] = [
            secrets.choice(uppercase_chars),   # at least one uppercase letter
            secrets.choice(lowercase_chars),   # at least one lowercase letter
            secrets.choice(digit_chars),        # at least one digit
            secrets.choice(special_chars),      # at least one special character
        ]

        # ------------------------------------------------------------------
        # Stage 2 — Fill: populate the remaining slots from the full printable
        # ASCII alphabet to maximise entropy.
        #
        # The full alphabet (ascii_letters + digits + punctuation) covers all
        # printable ASCII characters, which is the broadest character set we
        # can safely use.  Drawing fill characters from this wider set
        # increases the entropy per character compared to drawing only from
        # one of the category subsets.
        # ------------------------------------------------------------------
        full_alphabet: str = string.ascii_letters + string.digits + string.punctuation

        # Fill the remaining (length - 4) slots.  Each call to secrets.choice
        # is an independent, unpredictable draw from the OS CSPRNG.
        for _ in range(length - 4):
            chars.append(secrets.choice(full_alphabet))

        # ------------------------------------------------------------------
        # Stage 3 — Shuffle: randomise the order of all characters so that
        # the category-seeded characters (which are always the first four
        # before this step) are spread uniformly throughout the password.
        #
        # secrets.SystemRandom() returns a random.Random instance backed by
        # the OS CSPRNG, ensuring the shuffle is cryptographically
        # unpredictable.  A conventional random.shuffle would use the
        # Mersenne Twister PRNG, whose state could theoretically be
        # reconstructed from observed outputs — not acceptable for credentials.
        # ------------------------------------------------------------------
        secrets.SystemRandom().shuffle(chars)

        # Convert the shuffled character list to a single string and return.
        return "".join(chars)

    def _passes_weak_check(self, password: str) -> bool:
        """Return ``True`` if *password* does NOT appear in the weak-password list.

        A return value of ``True`` means the password *passed* the check (it
        is not weak); ``False`` means it *failed* (it was found in the weak
        list and should be discarded or replaced).

        The comparison is case-insensitive to prevent trivial bypasses such as
        "Password123" slipping through when "password123" is in the list.
        ...
        -------
        bool
            ``True``  — password is not in the weak list (safe to use).
            ``False`` — password matched a weak-list entry (should be replaced).
        """
        # Convert to lowercase once and compare against the already-lowercase
        # weak list entries.  A return of True means "passes" the check
        # (i.e., it is NOT in the weak list).
        return password.lower() not in self._weak_list
